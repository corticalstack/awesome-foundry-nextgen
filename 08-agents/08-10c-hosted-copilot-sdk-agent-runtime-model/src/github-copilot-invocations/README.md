...
# GitHub Copilot SDK - Invocations Protocol with runtime model selection

A minimal getting-started agent using the [GitHub Copilot SDK](https://pypi.org/project/github-copilot-sdk/) (`CopilotClient`) with the [azure-ai-agentserver-invocations](https://pypi.org/project/azure-ai-agentserver-invocations/) protocol. Streams raw Copilot SDK session events as SSE with multi-turn support, and lets each request switch the model or reasoning effort of the live session.

> Deploying this agent to Microsoft Foundry is driven by the lab notebook [`08-10c-01-runtime-model-and-telemetry.ipynb`](../../08-10c-01-runtime-model-and-telemetry.ipynb), which builds the container image with `az acr build` and registers it via the `azure-ai-projects` SDK. This README documents the agent itself and how to run it locally.
...
| `AZURE_AI_REASONING_EFFORT` | No | Default reasoning effort for new sessions: `low`, `medium` (default), `high`, or `xhigh` |
| `AZURE_AI_ALLOWED_MODELS` | No | Comma-separated deployments a request may switch to. Defaults to `AZURE_AI_MODEL_DEPLOYMENT_NAME` only |
...
### Switch the model or reasoning effort mid-conversation

Add `model` and/or `reasoning_effort` next to `input`. When they differ from the live session, `main.py` calls `session.set_model(...)` before sending the prompt. The change applies from that turn on and keeps the conversation history:

```bash
export AZURE_AI_ALLOWED_MODELS=gpt-5.4-nano,gpt-5.4-mini,gpt-5.4

curl -N -X POST http://localhost:8088/invocations \
  -H "Content-Type: application/json" \
  -d '{"input": "My name is Priya.", "model": "gpt-5.4-mini", "reasoning_effort": "medium"}'

# Same conversation, smaller and faster model
curl -N -X POST http://localhost:8088/invocations \
  -H "Content-Type: application/json" \
  -d '{"input": "What is my name?", "model": "gpt-5.4-nano", "reasoning_effort": "low"}'
```

A switch emits a `session.model_change` event (`previousModel`, `newModel`, `previousReasoningEffort`, `reasoningEffort`), and each model call emits `assistant.usage` with the `model` and `reasoningEffort` it actually used. A model outside `AZURE_AI_ALLOWED_MODELS` or an unknown effort returns HTTP 400.

A chat UI should send its current selection on every request, not only when it changes: after an idle timeout the hosted platform restarts the container, and the in-memory selection falls back to the defaults.
...
To change the persona, edit `system_prompt.md`, then rebuild and re-register the agent (the build and register steps of the notebook). If `system_prompt.md` is empty or missing, the CLI's default system message is used unchanged.
...
└── chat <model>                          (token usage, reasoning effort, estimated cost)
```
