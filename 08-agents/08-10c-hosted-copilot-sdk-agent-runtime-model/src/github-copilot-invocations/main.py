...
Runtime model selection (this lab, 08-10c):
  - The agent version pins a default model (AZURE_AI_MODEL_DEPLOYMENT_NAME) and
    reasoning effort (AZURE_AI_REASONING_EFFORT), plus the deployments a caller
    may switch to (AZURE_AI_ALLOWED_MODELS).
  - A request may add "model" and/or "reasoning_effort" next to "input". When they
    differ from the live session, main.py calls ``session.set_model(...)`` before
    sending the prompt. The switch applies from that turn on and keeps the
    conversation history. No new agent version, no redeploy.
  - Send the UI's current selection on every request: after an idle timeout the
    platform restarts the container and in-memory selection falls back to the
    defaults, while the conversation itself is restored from $HOME.
...
# Runtime model selection. The agent version pins a default model and reasoning
# effort through environment variables; a request can override either one, and
# the override applies to the live session (no new agent version, no redeploy).
DEFAULT_MODEL = os.environ.get("AZURE_AI_MODEL_DEPLOYMENT_NAME", "")
DEFAULT_REASONING_EFFORT = os.environ.get("AZURE_AI_REASONING_EFFORT", "medium")
# Deployments a caller may switch to. Anything else is rejected with HTTP 400.
ALLOWED_MODELS = [
    m.strip()
    for m in os.environ.get("AZURE_AI_ALLOWED_MODELS", DEFAULT_MODEL).split(",")
    if m.strip()
]
REASONING_EFFORTS = ("low", "medium", "high", "xhigh")
_current = {"model": DEFAULT_MODEL, "reasoning_effort": DEFAULT_REASONING_EFFORT}
...
def _byok_provider() -> tuple[ProviderConfig | None, str | None]:
    ...
    # The OpenAI v1 surface lives under /openai/v1/ (trailing slash required).
...
async def _stream_response(
    ...
    target = {
        "model": model or _current["model"],
        "reasoning_effort": reasoning_effort or _current["reasoning_effort"],
    }
    switch = target != _current
    ...
    with trace_invocation(invocation_id, _session_id, request_model) as on_trace:
        ...
        try:
            if switch:
                # Takes effect for the next message; conversation history is kept.
                # The session emits session.model_change, forwarded to the caller.
                await _session.set_model(
                    target["model"], reasoning_effort=target["reasoning_effort"])
                logger.info("Switched model: %s -> %s", _current, target)
    ...
    if reasoning_effort is not None and reasoning_effort not in REASONING_EFFORTS:
        return JSONResponse(
            ...
            content={
                ...
                "message": f"reasoning_effort must be one of {list(REASONING_EFFORTS)}",
            },
        )
    return StreamingResponse(
        _stream_response(
            request.state.invocation_id, input_text, model, reasoning_effort),
