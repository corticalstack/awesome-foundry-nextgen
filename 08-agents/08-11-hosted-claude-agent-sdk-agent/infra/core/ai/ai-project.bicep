...
resource aiAccount 'Microsoft.CognitiveServices/accounts@2025-06-01' = {
  ...
  // 2025-10-01-preview is required for Anthropic/Claude (partner) deployments:
  // they carry a `modelProviderData` Marketplace block (org/country/industry)
  // that older deployment API versions reject. OpenAI deployments simply omit
  // those optional fields, so this same loop still serves gpt-style models.
  @batchSize(1)
  resource seqDeployments 'deployments@2025-10-01-preview' = [
    for dep in (deployments ?? []): {
      ...
      properties: union(
        { model: dep.model },
        (dep.?modelProviderData != null) ? { modelProviderData: dep.modelProviderData! } : {},
        (dep.?versionUpgradeOption != null) ? { versionUpgradeOption: dep.versionUpgradeOption! } : {},
        (dep.?raiPolicyName != null) ? { raiPolicyName: dep.raiPolicyName! } : {}
      )
    }
  ]
...
type deploymentsType = {
  ...
  @description('Optional. Marketplace provider metadata required by partner (e.g. Anthropic/Claude) model deployments.')
  modelProviderData: {
  ...
  @description('Optional. Version upgrade policy for the deployment.')
  versionUpgradeOption: string?

  @description('Optional. Responsible AI policy name to apply to the deployment.')
